...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="page/page-canvas" mode="pre" xml:space="preserve"><xsl:apply-templates mode="pre"/></xsl:template>
		...
		<xsl:template match="p:*[not(@type)][not(local-name() = 'content')][not(key('template-local',local-name()))]" mode="pre">
			...
			<xsl:apply-templates select="key('template',local-name())[not(../following-sibling::templates/*[local-name() = local-name(current())])]" mode="template">
		...
		<xsl:template match="p:*[not(@type)][not(local-name() = 'content')][key('template-local',local-name())]" mode="pre">
			...
			<xsl:apply-templates select="key('template-local',local-name())[not(../following-sibling::templates/*[local-name() = local-name(current())])]" mode="template">
				<xsl:with-param name="id" select="$id"/>
			</xsl:apply-templates>
		</xsl:template>
		...
		<!--<xsl:template match="p:content" mode="pre_in">-->
			<!--<xsl:param name="id"/>-->
			<!--<xsl:variable name="parent-template" select="ancestor::*[parent::templates]"/>-->
			<!--<xsl:variable name="parent-template" select="name(ancestor::t:*)"/>-->
			<!--Нужно вернуться-->
			<!--Поним, что id =	<xsl:value-of select="$id"/>-->
			<!--значит вызвали на из name = <xsl:value-of select="name(key('template-callback',$id))"/>-->
				<!--<xsl:apply-templates select="key('template-callback',$id)" mode="back"/>-->
					<!--<xsl:with-param name="name" select="$parent-template"/>-->
				<!--</xsl:apply-templates>-->
		<!--</xsl:template>-->
	...
	<xsl:template match="e:*[not(@block) and ancestor::node()[a:*/@block='true' and not(descendant::node()[namespace-uri() = '&block-namespace;'][descendant-or-self::node() = current()])]]" mode="set-block">
		<xsl:param name="block-name" select="local-name(ancestor::node()[node()[namespace-uri() = $additional-namespace][@block = 'true']][1]/node()[namespace-uri() = $additional-namespace][@block = 'true'])" />
		<xsl:attribute name="block">
			<xsl:value-of select="$block-name"/>
		</xsl:attribute>
	</xsl:template>

	<xsl:template match="m:*[(namespace-uri(parent::node()) = '&block-namespace;') or (namespace-uri(parent::node()) = '&additional-namespace;')]" mode="set-block">
