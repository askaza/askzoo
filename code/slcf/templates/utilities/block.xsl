...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="b:* | e:* | a:* | m:* | alxc:*"></xsl:template>
	...
	<xsl:template match="b:* | e:*" mode="content">
		<xsl:apply-templates />
	</xsl:template>
	...
	<xsl:template match="m:*[parent::e:*]" mode="class">
		<xsl:param name="block-name" select="parent::node()/@block" />
